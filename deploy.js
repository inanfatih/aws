...
const os = require('os')
const client = require('node-scp2')
...
archive.append(fs.createReadStream(path.join(__dirname, '.env')), {
  name: '.env',
})
archive.append(fs.createReadStream(path.join(__dirname, 'index.js')), {
  name: 'index.js',
})
archive.append(fs.createReadStream(path.join(__dirname, 'package.json')), {
  name: 'package.json',
})
archive.append(fs.createReadStream(path.join(__dirname, 'package-lock.json')), {
  name: 'package-lock.json',
})
archive.append(fs.createReadStream(path.join(__dirname, 'webpack.config.js')), {
    ...
    myClient.on('transfer', (buffer, uploaded, total) => {
      if (!uploadLen || uploaded === total) {
        ...
        uploadBar = new ProgressBar(
          'Uploading [:bar] :percent :etas remaining',
          {
            complete: '=',
            incomplete: ' ',
            width: 20,
    ...
    client.scp(
      file,
      {
        ...
        privateKey: require('fs').readFileSync(
          path.join(os.homedir(), '.ssh', 'hamster_key'),
        ),
        path: '/home/bitnami/',
      },
      myClient,
